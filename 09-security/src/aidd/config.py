...
def _parse_bool_env(raw: str | None, *, default: bool) -> bool:
    """Разбор булевых флагов env; при пустом значении или непонятном — ``default``."""
    s = (raw or "").strip().lower()
    if not s:
        return default
    if s in ("0", "false", "no", "off"):
...
def _parse_rate_limit_window_seconds(raw: str | None, *, default: float = 60.0) -> float:
    """Скользящее окно (сек.) для TELEGRAM_TEXT_RATE_LIMIT_*; по умолчанию 60."""
    ...
    try:
        ...
        raise ValueError(
            "TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS must be a positive number "
            f"(typically 10–3600), got: {raw!r}"
        ) from e
    if v < 5.0 or v > 3600.0:
        raise ValueError(
            "TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS must be between 5 and 3600 "
            f"(got {v})"
        )
    return v


def _parse_rate_limit_max_messages(raw: str | None, *, default: int = 24) -> int:
    """Максимум текстовых сообщений за окно; по умолчанию 24 при окне 60 с."""
    if raw is None or not str(raw).strip():
        return default
    try:
        ...
        raise ValueError(
            "TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES must be an integer "
            f"between 1 and 500, got: {raw!r}"
        ) from e
    if v < 1 or v > 500:
        raise ValueError(
            "TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES must be between 1 and 500 "
            f"(got {v})"
        )
    return v
...
class AppConfig:
    ...
    def from_env() -> "AppConfig":
        ...
        mask_pan_outgoing = _parse_bool_env(
            os.environ.get("MASK_PAN_OUTGOING"), default=True
        )

        telegram_text_rate_limit_enabled = _parse_bool_env(
            os.environ.get("TELEGRAM_TEXT_RATE_LIMIT_ENABLED"), default=True
        )
        telegram_text_rate_limit_window_seconds = _parse_rate_limit_window_seconds(
            os.environ.get("TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS"), default=60.0
        )
        telegram_text_rate_limit_max_messages = _parse_rate_limit_max_messages(
        ...
        return AppConfig(
            ...
            mask_pan_outgoing=mask_pan_outgoing,
            telegram_text_rate_limit_enabled=telegram_text_rate_limit_enabled,
            telegram_text_rate_limit_window_seconds=(
                telegram_text_rate_limit_window_seconds
            ),
