...
- Webhook Telegram.
- Инструменты и интеграции вне зафиксированных в idea/vision: **`rag_search`**, **`convert_currency`**, MCP-инструменты **`search_products`**, **`currency_converter_mcp`**, **`loan_payment_mcp`**, **`open_credit_card`** (другие MCP-серверы, веб-поиск, боевые платёжные API и т.п.) без явного расширения idea/vision.
...
- **KISS / YAGNI:** один понятный поток «сообщение → история в агенте → при необходимости **`rag_search`**, **`search_products`** (если MCP доступен), **`convert_currency`** и/или **`currency_converter_mcp`**, **`loan_payment_mcp`**, **`open_credit_card`** (после HITL — §12) → финальный текст»; общая логика построения retriever/rerank — переиспользуется **`rag_search`** и оценкой, без абстрактного «движка агентов».
...
│   └── tasklist.md
├── prompts/
│   └── system.txt                  # системный промпт: rag_search, MCP-инструменты, open_credit_card; few-shot вызовов
├── src/
│   └── <package_name>/
│       ├── main.py
│       ├── config.py
...
- **Агент:** `create_agent(..., middleware=[…HumanInTheLoopMiddleware для open_credit_card…], tools=[rag_search, convert_currency, …mcp_tools], …)` (точный порядок middleware — как в референсе и документации LangChain для версии проекта), **`mcp_tools`** — результат **`await mcp_client.get_tools()`** при успешном подключении к **`mcp-bank-agent`**.
...
5. **`SHOW_SOURCES`:** из **текущего** пользовательского запроса собрать документы **только из сообщений после последнего `HumanMessage`** в истории: все **`ToolMessage`**, порождённые вызовами **`rag_search`** (**не** **`convert_currency`**, **не** MCP-инструменты **`search_products`** / **`currency_converter_mcp`** / **`loan_payment_mcp`** / **`open_credit_card`**), распарсить и объединить в перечень для отображения (KISS: стабильный список).

6. Системный промпт — **`SYSTEM_PROMPT_PATH`**: роль банковского ассистента; **жёсткие правила** и **few-shot** для **`rag_search`** (формулировки под фрагменты из PDF/JSON **`data/`**); **когда вызывать `search_products`** — вопросы по перечню продуктов, ставкам/условиям из публичного справочника, акциям, если RAG по документам не даёт ответа или явно не покрывает карточку продукта; **когда вызывать `currency_converter_mcp`** — нужен **актуальный** курс/пересчёт по данным ЦБ (при работающем MCP); **когда достаточно `convert_currency`** — быстрый ориентир, MCP недоступен, или достаточно прежнего локального источника курсов; **когда вызывать `loan_payment_mcp`** — пользователь просит **посчитать** месячный платёж/переплату при **явно заданных** (или согласованных после уточнения) сумме кредита, годовой ставке и сроке в месяцах и при доступном MCP; при отсутствии числовых параметров — уточнить, не выдумывать; **когда вызывать `open_credit_card`** — явное намерение оформить/открыть кредитную карту (при доступном MCP), с напоминанием что перед выполнением пользователь подтверждает операцию в чате; условия договора и юридические формулировки — через **`rag_search`**, расчёт и мок-операции — не замена документам. Примеры вызовов MCP-инструментов — в промпте кратко, по аналогии с **`data/agent-mcp.ipynb`**.

7. **Telegram и HITL:** при **`__interrupt__`** для **`open_credit_card`** отправить пользователю сообщение с **деталями операции** (инструмент, аргументы, текст из **`action_requests`** / описание middleware) и **inline-кнопки** «Accept» и «Reject». После нажатия — **снять inline-клавиатуру** с этого сообщения и выполнить **resume** (§12).

8. **Маскирование перед отправкой в чат:** исходящие пользователю тексты проходят слой **PII** (§12); это не меняет контракт **`open_credit_card`** на стороне MCP (там PAN **без** маски). Переключатель **`MASK_PAN_OUTGOING`** — §9 и **`.env.example`**.
...
Общее: **`OPEN_API_KEY`**, **`OPEN_BASE_URL`**, модель чата, **`SYSTEM_PROMPT_PATH`**, **`SHOW_SOURCES`**, опционально **`MASK_PAN_OUTGOING`** (маска PAN перед отправкой в Telegram), **`TELEGRAM_TEXT_RATE_LIMIT_ENABLED`** (**включение/выключение** антиспама по текстовым репликам), **`TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS`**, **`TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES`**, **`LANGSMITH_*`**.
...
**Human-in-the-loop (`open_credit_card` только):** в **`interrupt_on`** middleware — только имя инструмента **`open_credit_card`**. Поддерживаемые решения пользователя: **принять** (**approve** / кнопка **Accept**) и **отклонить** (**reject** / **Reject**). Сценарий **edit** (правка аргументов перед вызовом) **не** реализуется (**YAGNI**).
...
**Rate limiting:** ограничить частоту **входящих текстовых сообщений** на **`chat_id`** (**скользящее окно** — метки времени в памяти процесса, глобальная блокировка **async**). **Включение и выключение целиком** — переменная **`TELEGRAM_TEXT_RATE_LIMIT_ENABLED`** (**§9**): при **`false`** (и эквивалентах из **`.env.example`**) счётчик **не** ведётся, **`TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS`** и **`TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES`** не применяются. При включённом лимите при превышении — короткий отказ **без** вызова LLM; пороги — **§9** и **`.env.example`**. Интерактивные команды бота (**`/start`**, **`/evaluate_dataset`**, **`/index`**, callback HITL) этим лимитом **не** обрезаются: он действует только на обычные текстовые реплики в диалоге. Опционально согласовать верхнюю границу **вызовов инструментов** за ход (**YAGNI** — достаточно лимита сообщений).

**Graceful degradation:** при недоступном MCP инструмент **`open_credit_card`** отсутствует; HITL для него не активируется.
...
| MCP | **`mcp/mcp-bank-agent`**, streamable HTTP **:8000**; инструменты включая **`open_credit_card`**; **`langchain-mcp-adapters>=0.1.0`**; async **`create_bank_agent`**, **`initialize_agent`**; **`mcp_tools = await mcp_client.get_tools()`** |
| Чувствительная операция | MCP **`open_credit_card`** — мок; **HITL** только **Accept/Reject** (§12); контракт MCP — PAN без маски; в чат и логи бота — маскирование (§12) |
| PII / лимиты | **PAN**: **`MASK_PAN_OUTGOING`**; антиспам по тексту: **`TELEGRAM_TEXT_RATE_LIMIT_ENABLED`**, при вкл.: **`TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS`**, **`TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES`** (§12, **`.env.example`**) |
