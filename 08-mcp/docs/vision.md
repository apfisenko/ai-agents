...
- Webhook Telegram.
- Инструменты и интеграции вне зафиксированных в idea/vision: **`rag_search`**, **`convert_currency`**, MCP-инструменты **`search_products`**, **`currency_converter_mcp`** и **`loan_payment_mcp`** (другие MCP-серверы, веб-поиск, действия в банковских системах и т.п.) без явного расширения idea/vision.
...
- **KISS / YAGNI:** один понятный поток «сообщение → история в агенте → при необходимости **`rag_search`**, **`search_products`** (если MCP доступен), **`convert_currency`** и/или **`currency_converter_mcp`**, **`loan_payment_mcp`** (при запросе ориентировочного платежа по параметрам кредита) → финальный текст»; общая логика построения retriever/rerank — переиспользуется **`rag_search`** и оценкой, без абстрактного «движка агентов».
...
│   └── tasklist.md
├── prompts/
│   └── system.txt                  # системный промпт: rag_search, search_products, convert_currency, currency_converter_mcp, loan_payment_mcp; few-shot
├── src/
│   └── <package_name>/
│       ├── main.py
│       ├── config.py
