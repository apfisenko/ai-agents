...
def main() -> None:
    ...
    logger.info(
        "RAG retrieval: mode=%s, embedding_provider=%s, semantic_k=%s",
    ...
    if config.mcp_bank_enabled:
        logger.info(
            "MCP bank: enabled, streamable HTTP URL %s",
            config.mcp_bank_streamable_http_url,
        )
    else:
        logger.info("MCP bank: disabled (MCP_BANK_ENABLED=false)")
...
async def _run(config: AppConfig) -> int:
    app = await TelegramBot.create(config)
    try:
        logger.info("Building vector index at startup…")
        await app.bootstrap_vector_index()
    except Exception:
        logger.exception("Startup indexing failed; exiting.")
        await app.close()
        return 1
    try:
