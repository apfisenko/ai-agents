...
**Продуктовая роль (содержание ответов):** по [idea.md](idea.md) бот позиционируется как **консультант по путешествиям**: страны, достопримечательности, интересные места, народные традиции и близкие темы. Это достигается текстом системного промпта (файл по `SYSTEM_PROMPT_PATH`), **без** отдельной БД, RAG и специализированных обработчиков под темы — границы MVP не расширяются.
...
├── Dockerfile
├── .env.example
└── make.ps1                      # опционально: дубль целей Makefile для PowerShell (.\make.ps1 install | run)
```
...
- **Лимит длины ответа:** переменная **`LLM_MAX_COMPLETION_TOKENS`** (целое **64–8192**, обязательное в **`.env`**): передаётся в API как `max_tokens` для chat completions и ограничивает длину ответа модели.
...
| Сценарий | Поведение |
|----------|-----------|
| **Старт** | Команда `/start` (или аналог): краткое приветствие **в духе роли** (консультант по путешествиям задаётся системным промптом; при необходимости согласовать формулировки в handler с промптом); при желании — сброс истории для данного `chat_id` (явное правило зафиксировать в коде: сбрасывать или нет). |
...
- **Обязательные переменные:** токен Telegram-бота; ключ API OpenRouter; идентификатор/имя модели; **`OPENROUTER_BASE_URL`** — базовый URL API (вызовы LLM не хардкодятся в коде); **`SYSTEM_PROMPT_PATH`** — путь к **файлу** с текстом системного промпта (содержимое читается при старте или при первом обращении; сам текст промпта в env не дублируем, только путь); **`LLM_MAX_COMPLETION_TOKENS`** — максимальное число токенов в ответе модели (**64–8192**), передаётся в совместимый вызов chat completions как `max_tokens`. В **`.env.example`** перечислить все обязательные имена и примеры значений (без секретов), для `OPENROUTER_BASE_URL` указать типичное значение `https://openrouter.ai/api/v1`; для **`LLM_MAX_COMPLETION_TOKENS`** указать типичное значение (например **1024**).
...
- **PowerShell:** те же цели, что в Makefile — скриптом `make.ps1` в корне рядом с `Makefile` (например `.\make.ps1 run`), чтобы на Windows можно было обойтись без GNU Make.
...
| Лимит ответа LLM | **`LLM_MAX_COMPLETION_TOKENS`** в **`.env`** (целое **64–8192**), в API — `max_tokens` |
...
| Роль в диалоге | Консультант по путешествиям и смежным темам — через **системный промпт**, без новых компонентов в MVP |
