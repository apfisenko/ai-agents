...
# Дефолт для EMBEDDING_PROVIDER=huggingface (репозиторий HF, не API slug)
DEFAULT_HF_EMBEDDING_MODEL: Final[str] = "intfloat/multilingual-e5-base"
...
EmbeddingProviderName = Literal["openai", "huggingface"]
...
def make_huggingface_embeddings(*, embedding_model: str) -> Embeddings:
    """Локальные эмбеддинги через langchain-huggingface (sentence-transformers)."""
    from langchain_huggingface import HuggingFaceEmbeddings as HFCompatEmbeddings

    return HFCompatEmbeddings(
        model_name=embedding_model,
        encode_kwargs={"normalize_embeddings": True},
    )


def _embeddings_for_provider(
    *,
    ...
    if embedding_provider == "huggingface":
        return make_huggingface_embeddings(embedding_model=embedding_model)
...
def build_vector_store(
    *,
    ...
    chunk_size: int = DEFAULT_CHUNK_SIZE,
    chunk_overlap: int = DEFAULT_CHUNK_OVERLAP,
) -> tuple[InMemoryVectorStore, list[Document], int]:
    """
    Полный пайплайн: PDF → сплит чанков; JSON (`full_text`) → по одному документу на запись;
    затем эмбеддинги → InMemoryVectorStore.

    Список `splits` — те же чанки, что ушли в векторное хранилище (для BM25 / hybrid).

    Returns:
        (store, splits, num_chunks)
    """
    ...
    logger.info(
        "Эмбеддинги: provider=%s model=%s", embedding_provider, embedding_model
    )
    embeddings = _embeddings_for_provider(
    ...
    if embedding_provider == "openai":
        try:
            ...
            raise RuntimeError(
                f"Ошибка подключения к API эмбеддингов при индексации. {hint}"
            ) from e
    else:
        try:
            store = InMemoryVectorStore.from_documents(splits, embeddings)
            ...
            raise RuntimeError(
                "Ошибка при построении векторного индекса (HuggingFace / sentence-transformers). "
                "Проверьте EMBEDDING_MODEL (id модели), память и при необходимости докачку весов."
            ) from e
    return store, splits, len(splits)
