...
- Скопировать `.env.example` в `.env` и задать переменные (токен бота, ключ в `OPEN_API_KEY`, модель, `OPEN_BASE_URL`, **`RETRIEVER_K`** (RAG), лимит ответа `LLM_MAX_COMPLETION_TOKENS`, путь к системному промпту и т.д.).
