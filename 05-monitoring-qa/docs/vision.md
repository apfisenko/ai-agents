...
**Мониторинг и качество:** при необходимости бот может **показывать источники** найденных фрагментов (файл, страницы); запросы **трейсятся в LangSmith** через переменные окружения (поддержка LangChain без доработки кода пайплайна). Для регрессий и экспериментов — **синтез локального датасета Q&A**, загрузка наборов в LangSmith и **оценка ответов метриками RAGAS** с отправкой результатов как feedback в LangSmith (подробности в разделе про мониторинг ниже).
...
Эталон логики RAG — ноутбук **`data/naive-rag.ipynb`**, финальная цепочка **`rag_query_transform_chain`**; эталон сценариев оценки — **`data/rag-evaluation-practice.ipynb`** (в т.ч. подход к RAGAS):
...
| Наблюдаемость датасетов и экспериментов | **LangSmith** (env **`LANGSMITH_*`**), **`langsmith`**, **`datasets`**, **`ragas`** (≥ **0.2.0**) | Трейсинг RAG включается окружением; оценки — отдельным модулем и командой бота. |
...
│   ├── rag-evaluation-practice.ipynb  # референс оценки / RAGAS
│   ├── *.pdf
...
│       ├── dataset_synthesizer.py   # синтез Q&A и подготовка датасета (утилита / make)
│       └── evaluation.py            # прогон RAGAS и выгрузка метрик (команда бота + утилита)
├── datasets/                        # сгенерированные датасеты (например 05-rag-qa-dataset.json)
├── pyproject.toml
├── uv.lock
├── Makefile
├── docker-compose.yml
...
- **RAG chain:** query transformation (история → поисковая строка) → retriever **top-K** (`RETRIEVER_K`) → контекст → ответ LLM с историей; цепочка должна **уметь отдавать вместе с текстом ответа** список использованных **документов с метаданными** (имя файла, номера страниц), чтобы при **`SHOW_SOURCES=true`** хендлер мог вывести пользователю строку вида: `📚 Источники: filename.pdf (стр. 1, 3, 5)`.
...
| **`/evaluate_dataset`** | полный цикл **оценки датасета метриками RAGAS** (§10) и загрузка результатов **как feedback в LangSmith**; требуются настроенные ключи/проект LangSmith и параметры моделей для RAGAS. |
...
- **`SHOW_SOURCES`** — если включено (**`true`**), к ответу пользователю добавляется блок **источников** в согласованном формате (см. поток RAG выше).
- **LangSmith:** стандартные **`LANGSMITH_*`** (в т.ч. API key, project, tracing) — **включают трейсинг цепочек LangChain** без изменений кода RAG; перечень имён поддерживать в **`.env.example`**.
- **RAGAS:** **`RAGAS_LLM_MODEL`**, **`RAGAS_EMBEDDING_MODEL`** — модели для метрик (OpenRouter / OpenAI-совместимый клиент в духе §3); остальное — как требует `ragas` и выбранный провайдер.

---

## 10. Мониторинг, синтез датасетов и RAGAS

1. **Источники в ответе:** управляется **`SHOW_SOURCES`**; RAG-цепочка возвращает **ответ + retrieved-документы** для форматирования (не дублировать логику поиска в хендлере).
2. **LangSmith:** достаточно корректно задать переменные окружения — **отдельный код трейсинга в цепочке не писать**, если поведение покрывается LangChain.
3. **Синтез датасета (`dataset_synthesizer.py`):** из каждого **PDF** в **`data/`** взять **по 2 чанка**; для каждого чанка **LLM** генерирует пару **вопрос — ответ**; при наличии готовых Q&A в **JSON** — **объединить** с синтезом; итог сохранять в **`datasets/05-rag-qa-dataset.json`**; опционально **загрузка в LangSmith** с **пропуском дубликатов**. Сборка: цели **`make dataset`** и **`make dataset-upload`** (без лишних целей).
4. **Оценка (`evaluation.py`):** по команде **`/evaluate_dataset`** — прогон полного цикла оценки на датасете; метрики **faithfulness**, **answer_relevancy**, **answer_correctness**, **answer_similarity**, **context_recall**, **context_precision**; агрегаты/прогоны отражать в **LangSmith как feedback**. Ориентир по сценарию — раздел 5 в **`data/rag-evaluation-practice.ipynb`**.
...
Как в текущем проекте: **`uv sync`**, **`uv run`** / Makefile, Docker Compose с одним сервисом, при необходимости дубли команд в PowerShell. В Makefile добавляются цели **`dataset`** и **`dataset-upload`** для пайплайна датасета (см. §10). Продакшен-деплой и CI в этом документе не фиксируются.
...
| Источники в UI | **`SHOW_SOURCES`**, цепочка возвращает документы для строки «📚 Источники: …» |
| Трейсинг | **LangSmith** через **`LANGSMITH_*`**, без обязательного кода в цепочке |
| Датасет / RAGAS | **`dataset_synthesizer.py`**, **`datasets/05-rag-qa-dataset.json`**, **`make dataset`**, **`make dataset-upload`**; **`evaluation.py`**, **`/evaluate_dataset`**, шесть метрик RAGAS, feedback в LangSmith |
| Референс RAG | **`data/naive-rag.ipynb`** → **`rag_query_transform_chain`** |
| Референс оценки | **`data/rag-evaluation-practice.ipynb`** (раздел 5) |
