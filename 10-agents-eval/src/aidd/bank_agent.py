...
class BankAgentRunner:
    ...
    async def aget_thread_messages(self, thread_id: str) -> list[BaseMessage]:
        """Сообщения состояния графа для ``thread_id`` (траектория для e2e / agentevals)."""
        snap = await self._agent.aget_state({"configurable": {"thread_id": thread_id}})
        vals = snap.values
        raw = vals.get("messages") if isinstance(vals, dict) else None
        return list(raw or [])
