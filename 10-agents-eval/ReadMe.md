...
- Скопировать `.env.example` в `.env` и задать переменные (токен бота, ключ в `OPEN_API_KEY`, модель, `OPEN_BASE_URL`, **`RETRIEVER_K`** (RAG), лимит ответа `LLM_MAX_COMPLETION_TOKENS`, путь к системному промпту; опционально **`MASK_PAN_OUTGOING`**, **`TELEGRAM_TEXT_RATE_LIMIT_ENABLED`** (+ окно и лимит при включённом rate limit — **`TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS`**, **`TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES`**), лимит **ходов агента** **`AGENT_INVOCATIONS_RATE_LIMIT_ENABLED`** (+ **`WINDOW_AGENT_SECONDS`**, **`MAX_AGENT_INVOCATIONS_PER_WINDOW`** при включении) — см. раздел ниже и `docs/vision.md`).
- **Напрямую:** `uv sync` и `uv run python -m aidd` (из корня каталога проекта `02-aidd`, где лежит `pyproject.toml`).
- **macOS и Linux** — GNU Make в корне проекта или **`bash make.sh <цель>`** (те же задачи для POSIX / Git Bash, см. `bash make.sh help`):
  - `make install` или `bash make.sh install` — `uv sync`
  - `make run` или `bash make.sh run` — `uv run python -m aidd`
...
**macOS / Linux:** те же действия через `make docker-build`, `make docker-up`, `make docker-down`, `make docker-ps`, `make docker-check` или `bash make.sh docker-*` при тех же задачах (локальный `docker compose`).
...
**Обязательные параметры включают:** `TELEGRAM_BOT_TOKEN`, `OPEN_API_KEY`, `LLM_MODEL`, `OPEN_BASE_URL`, **`RETRIEVER_K`** (целое **1–50**, топ-K для будущего retriever в RAG), **`LLM_MAX_COMPLETION_TOKENS`** — целое **64–8192**, задаёт **`max_tokens`** для ответа модели (типично **1024**); `SYSTEM_PROMPT_PATH` и др. По желанию: `LOG_LEVEL`, `EMBEDDING_MODEL`, **`MASK_PAN_OUTGOING`** — маска **PAN** (16–19 цифр) в текстах перед отправкой в Telegram (ответ агента, HITL, сводка `/evaluate_dataset`; по умолчанию вкл.; см. `docs/vision.md` §9, §12), **`TELEGRAM_TEXT_RATE_LIMIT_ENABLED`** — включить (**`true`/пусто**) или полностью отключить (**`false`**) ограничение частоты **текстовых** сообщений без LLM при спаме (**`plain_text`**; команды бота не в счёт); при включённом лимите задают порог **`TELEGRAM_TEXT_RATE_LIMIT_WINDOW_SECONDS`**, **`TELEGRAM_TEXT_RATE_LIMIT_MAX_MESSAGES`**; **`AGENT_INVOCATIONS_RATE_LIMIT_ENABLED`** — отдельный лимит числа **`ainvoke_turn`** на **`chat_id`** (в т.ч. resume HITL); при включении — **`WINDOW_AGENT_SECONDS`**, **`MAX_AGENT_INVOCATIONS_PER_WINDOW`** (см. `docs/vision.md` §12), `DATA_DIR`, `TELEGRAM_HTTP_TIMEOUT`, прокси (`HTTPS_PROXY` / `HTTP_PROXY`).
