...
param(
    ...
    [ValidateSet("install", "run", "run-mcp-bank", "stop-mcp-bank", "check-mcp-bank", "check-bot", "stop-bot", "smoke-index", "smoke-rag-chain", "dataset", "dataset-upload", "test-e2e-agent", "test-e2e-agent-deterministic", "test-e2e-agent-judge", "docker-build", "docker-up", "docker-down", "docker-up-host", "docker-down-host", "docker-ps", "docker-check", "docker-windows-host-ip", "docker-portproxy-hint", "portproxy-up", "portproxy-down", "help")]
    [string] $Target = "help",
...
switch ($Target) {
    ...
    "run-mcp-bank" {
        $mcpDir = Join-Path $PSScriptRoot "mcp\mcp-bank-agent"
    ...
    "check-mcp-bank" { & uv run python -m aidd.mcp_health check }
    "stop-mcp-bank" { & uv run python -m aidd.mcp_stop }
    "check-bot" { & uv run python -m aidd.bot_processes check }
    "stop-bot" { & uv run python -m aidd.bot_processes stop }
    ...
    "help" {
        ...
        Write-Host "  run-mcp-bank  - MCP mcp-bank-agent (mcp/mcp-bank-agent: uv sync + mcp-bank-agent); терминал 2: run"
        Write-Host "  stop-mcp-bank - завершить процесс на порту MCP (см. MCP_BANK_PORT / URL в .env; на порту может быть другой сервис)"
        Write-Host "  check-mcp-bank - проверка MCP по MCP_BANK_STREAMABLE_HTTP_URL (.env), JSON + exit code"
        Write-Host "  check-bot     - «python -m aidd» (PID/PPID); на Windows 2 процесса из .venv = часто один бот (лаунчер + интерпретатор); exit 1 если логических экземпляров >1"
        Write-Host "  stop-bot      - завершить все найденные «python -m aidd» (Windows: taskkill /F); затем один раз .\make.ps1 run"
        ...
        Write-Host "  dataset       - синтез JSON в datasets/ (имя файла из LANGSMITH_DATASET; OPEN_*, LLM_*; vision §10)"
        Write-Host "  dataset-upload - выгрузка JSON в LangSmith (LANGSMITH_API_KEY, LANGSMITH_DATASET)"
        Write-Host "  test-e2e-agent   - pytest tests/e2e (agentevals; uv sync --group dev)"
        Write-Host "  test-e2e-agent-deterministic - только подбор траектории (без LLM-судьи)"
        Write-Host "  test-e2e-agent-judge - только LLM-as-a-Judge (нужен AGENTEVALS_LLM_MODEL)"
