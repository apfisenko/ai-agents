...
@mcp.tool
def open_credit_card(application_note: str | None = None) -> str:
    """Эмуляция (мок) открытия кредитной карты для демонстрации сценария в ассистенте.

    Не вызывает реальных банковских систем и не создаёт юридических обязательств.
    application_note — необязательная краткая пометка из диалога (например тип продукта).

    Возвращает JSON: при успехе ok=true, operation_id, card_number (полный номер карты,
    без маскирования — имитация ответа ядра), expiry (MM/YY), payment_system, credit_limit_rub,
    currency, note; при ошибке ok=false, error.
    """
    try:
        payload = issue_mock_credit_card(application_note)
        pan = str(payload["card_number"])
        log.info(
            "open_credit_card mock issued operation_id=%s pan_last4=%s",
