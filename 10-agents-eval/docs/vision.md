...
**Оценка траекторий агента (agentevals):** **`data/agent-evaluation.ipynb`** — извлечение траектории из графа, **`create_async_trajectory_match_evaluator`** (сопоставление с эталоном по шагам / вызовам инструментов), **`create_async_trajectory_llm_as_judge`** (оценка осмысленности траектории LLM-судьёй). E2E в репозитории: **`tests/e2e/`**, цели **`make test-e2e-agent`** и см. §10 п. 5, §13.
...
├── datasets/
├── tests/
│   └── e2e/                        # e2e агента: agentevals (детерминированное сопоставление + LLM-judge)
│       ├── conftest.py
│       ├── test_agent_e2e_deterministic.py
│       └── test_agent_e2e_llm_judge.py
├── pyproject.toml                  # зависимость langchain-mcp-adapters>=0.1.0; группа dev: pytest, agentevals
├── uv.lock
├── Makefile                        # run, run-mcp-bank, test-e2e-agent, …
├── make.ps1
...
**E2E траектории (agentevals):** модель судьи для **`create_async_trajectory_llm_as_judge`** — **`AGENTEVALS_LLM_MODEL`** (OpenRouter-идентификатор, тот же **`OPEN_BASE_URL`** / **`OPEN_API_KEY`**, что у проекта). Без переменной тест с судьёй пропускается (**skip**). Зависимости прогона — группа **dev** в **`pyproject.toml`** (**`uv sync --group dev`**).
...
5. **E2E тесты агента (`tests/e2e/`, agentevals):** проверяют **полный ход** банковского агента (индекс, **`BankAgentRunner.ainvoke_turn`**, сообщения из состояния графа), без Telegram. Две группы: **детерминированное сопоставление траектории** — **`create_async_trajectory_match_evaluator`** (в проекте: режим **superset** + **ignore** аргументов инструмента, эталон задаёт ожидаемый вызов **`rag_search`** или **`convert_currency`**); **LLM-as-a-Judge** — **`create_async_trajectory_llm_as_judge`** с промптом **`TRAJECTORY_ACCURACY_PROMPT`** (без эталонной траектории в промпте). Референс по смыслу и API — **`data/agent-evaluation.ipynb`**. В прогоне e2e **MCP отключается** (**`MCP_BANK_ENABLED=false`** в фикстуре), чтобы набор инструментов был стабильным (**`rag_search`**, **`convert_currency`**); для индекса в фикстуре принудительно **`EMBEDDING_PROVIDER=openai`** и **`RAG_RETRIEVAL_MODE=semantic`**, чтобы не тянуть модели с Hugging Face и не зависеть от cross-encoder при сбоях прокси к HF. Запуск: **`make test-e2e-agent`** (все), **`make test-e2e-agent-deterministic`**, **`make test-e2e-agent-judge`**; локально — **`uv sync --group dev`**, затем **`uv run pytest tests/e2e`**. Полный набор переменных окружения как для бота (**`AppConfig.from_env`**); для судьи — **`AGENTEVALS_LLM_MODEL`** (§9).
...
**`uv sync`**, **`uv run`** / Makefile, Docker Compose с одним сервисом приложения (по-прежнему). Цели **`dataset`** и **`dataset-upload`**. E2E агента: **`test-e2e-agent`**, **`test-e2e-agent-deterministic`**, **`test-e2e-agent-judge`** (перед прогоном **`uv sync --group dev`** внутри целей Make); дубли в **`make.ps1`** / **`make.sh`** (как для **`run`**, **`run-mcp-bank`**, …).
...
| E2E агента | **`tests/e2e/`**, agentevals: match-evaluator + LLM-judge; **`AGENTEVALS_LLM_MODEL`**; **`make test-e2e-agent`** |
